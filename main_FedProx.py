...
# Define custom FedProx loss function
def fedprox_loss(model, global_weights, mu=0.01):
    def loss_fn(y_true, y_pred):
        # Original loss
        original_loss = tf.reduce_mean(tf.square(y_true - y_pred))
        
        # Proximal term
        proximal_term = 0.0
        for w, w_t in zip(model.trainable_weights, global_weights):
            proximal_term += tf.reduce_sum(tf.square(w - w_t))
        
        # Combined loss with proximal term
        total_loss = original_loss + (mu / 2.0) * proximal_term
        return total_loss
    return loss_fn
...
for k in range(1, len(t)):
    for node in range(num_nodes):
        ...
        # Train LSTM model every 50 steps with FedProx
        if k % 50 == 0:
            # Get weights from other nodes for FedProx
            other_weights = [lstm_models[other_node].get_weights() 
                             for other_node in range(num_nodes) 
                             if other_node != node]
            
            # Calculate global weights (average of other nodes)
            global_weights = [np.mean([weights[i] for weights in other_weights], axis=0) 
                              for i in range(len(lstm_model.get_weights()))]
            
            # Set global weights temporarily and recompile with FedProx loss
            mu = 0.01  # Proximal term coefficient
            lstm_model.compile(
                optimizer=Adam(learning_rate=0.001),
                loss=fedprox_loss(lstm_model, global_weights, mu)
            )
            
            # Train the model
            target = np.zeros((z_input.shape[0], output_dim))
