...
def ekf(x, z, P, Q, R, motion_model, measurement_model, sensor_pos, delta, alpha=1.0):
    ...
    try:
        ...
        print("Warning: SVD did not converge. Using identity matrix for Kalman gain.")
        K = np.eye(P_pred.shape[0])  # Fallback to identity matrix
...
for k in range(1, len(t)):
    for node in range(num_nodes):
        ...
        if k % 50 == 0:
            # Adaptive weighting for averaging weights from other nodes using FedNova
            other_weights = [lstm_models[other_node].get_weights() for other_node in range(num_nodes) if other_node != node]
            local_steps = np.array([len(training_losses[other_node]) for other_node in range(num_nodes) if other_node != node])
            adaptive_weights = []
            for layer in range(len(other_weights[0])):
                layer_weights = np.array([weights[layer] for weights in other_weights])
                if np.sum(local_steps) > 0:  # Ensure no division by zero
                    normalized_steps = local_steps / np.sum(local_steps)  # Normalize local steps
                else:
                    normalized_steps = np.ones_like(local_steps) / len(local_steps)  # Equal weights if no steps
                adaptive_layer_weights = np.tensordot(normalized_steps, layer_weights, axes=(0, 0))
                adaptive_weights.append(adaptive_layer_weights)
            lstm_model.set_weights(adaptive_weights)

            # Train the model
            if not np.isnan(z_input).any():  # Skip training if input contains nan
                epochs = 10
